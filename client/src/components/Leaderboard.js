...
const Leaderboard = ({ records, data, currentData, handleLeaderboardClick, leaderboardIsHidden }) => {
  ...
  if (records.gamesPlayed >= 1 || (data.length > 0 && currentData.length >= data.length-1)) {
    ...
    hamburger =
      <div id='hamburger' className={hamburgerClass} onClick={() => {handleLeaderboardClick()}}>
        <div class="bar1"></div>
        <div class="bar2"></div>
        <div class="bar3"></div>
      </div>;
